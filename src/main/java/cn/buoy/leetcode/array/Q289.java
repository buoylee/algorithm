...
public class Q289 {
    ...
    public void gameOfLife(int[][] board) {
        ...
        for (int i = 0; i < rowLen; i++) {
            for (int j = 0; j < colLen; j++) {
                ...
                // 不用看这行注释. 屬於第一個視頻相關. 和视频不一样, 视频是直接处理定义好的表示从`死到活`, `活到死`的2个新state,完成状态变化后, 在恢复2个新状态到0, 1.
    ...
    public int liveNeighbors(int[][] board, int rowLen, int colLen, int i, int j) {
        int lives = 0;
        // Math.max(i - 1, 0) 上边界防越界
        // Math.min(i + 1, rowLen - 1) 下边界防越界
        for (int x = Math.max(i - 1, 0); x <= Math.min(i + 1, rowLen - 1); x++) {
            // Math.max(j - 1, 0) 左边界防越界
            // Math.max(j + 1, colLen - 1) 右边界防越界
            for (int y = Math.max(j - 1, 0); y <= Math.min(j + 1, colLen - 1); y++) {
                // 如果 Neighbor value 的 "末尾 bit" 是 1, lives++
                lives += board[x][y] & 1;
            }
        }
        // 记得! 如果 "本 node" 是 live, 扣除自己.
        lives -= board[i][j] & 1;
        return lives;
    }
}
