...
public class Q188 {
     ...
     * <p>
     * 注意: "一次trx" 等于 "1次买和卖", "上一次的卖" 可以和 "下一次的买" 同一天; 但是 "本次的 买卖" 不能是 同一天.
     * <p>
...
//        if (len == 0) return 0;
        // 一次trx 等于1次 "买和卖", 所以足够每次在低位买, 高位卖. 所以, 简化为, 无限制交易次数的题解.
        if (k >= len / 2) return quickSolve(prices);
        //dp[currTrx][currDay]表示 在完成 第 currTrx 次交易在第 currDay 天时, 最大的收益.
        ...
        for (int currTrx = 1; currTrx <= k; currTrx++) {
            ...
            for (int currDay = 1; currDay < len; currDay++) {
                ...
                // 2. tmpMax(之前某一天上次交易 urrTrx-1(currTrx 現在還沒賣出, 所以只能算 currTrx-1)) 加上 "当天賣出的price" 的最大值.
                dp[currTrx][currDay] = Math.max(dp[currTrx][currDay - 1], prices[currDay] + tmpMax);
                // 先看懂这行
                // 某前一天最大收益 + 当天j 买入 扣除的价钱, 和以往比找出最大值.
                // 关键: tmpMax 表示, 在 currDay 的 `到上次交易为止` 且在 currDay 天买入, 但不賣出 的最大收益, 這個並不是 dp, 而是爲了計算 dp, 所需的 "未賣出時的最大收益"
